...
Sistem BISINDO AI ini dibuat oleh Ricy Ikada Saputra dari Universitas Mercu Buana, jurusan Artificial Intelligence 2026.
...
GESTURE_DESC = {
    "A": "Tangan kanan: kepalan, ibu jari di samping · Tangan kiri: telapak terbuka mendatar",
    "B": "Tangan kanan: empat jari lurus ke atas · Tangan kiri: menopang pergelangan",
    "C": "Kedua tangan melengkung membentuk huruf C saling berhadapan",
    "D": "Tangan kanan: telunjuk lurus, jari lain lingkaran · Tangan kiri: telapak menghadap dalam",
    "E": "Kedua tangan: semua jari ditekuk ke bawah, telapak menghadap depan",
    "F": "Tangan kanan: ibu jari & telunjuk lingkaran, jari lain lurus · Tangan kiri: menopang",
    "G": "Tangan kanan: telunjuk & ibu jari sejajar ke samping · Tangan kiri: mengepal di bawah",
    "H": "Kedua tangan: telunjuk & jari tengah sejajar horizontal saling menunjuk",
    "I": "Tangan kanan: kelingking lurus ke atas · Tangan kiri: mengepal di bawah",
    "J": "Tangan kanan: kelingking lurus lalu gambar J · Tangan kiri: telapak terbuka",
    "K": "Tangan kanan: telunjuk ke atas, jari tengah diagonal · Tangan kiri: menopang",
    "L": "Tangan kanan: ibu jari & telunjuk membentuk L · Tangan kiri: mengepal di samping",
    "M": "Kedua tangan: tiga jari ditekuk menutupi ibu jari, telapak menghadap bawah",
    "N": "Kedua tangan: dua jari ditekuk menutupi ibu jari, saling berhadapan",
    "O": "Kedua tangan: semua jari membentuk lingkaran O saling berhadapan",
    "P": "Tangan kanan: seperti K mengarah bawah · Tangan kiri: telapak horizontal",
    "Q": "Tangan kanan: seperti G mengarah bawah · Tangan kiri: mengepal di samping",
    "R": "Tangan kanan: telunjuk & jari tengah bersilang · Tangan kiri: mengepal",
    "S": "Kedua tangan: kepalan dengan ibu jari di depan jari, saling berhadapan",
    "T": "Tangan kanan: ibu jari di antara telunjuk & jari tengah · Tangan kiri: menopang",
    "U": "Kedua tangan: telunjuk & jari tengah lurus bersama, sejajar",
    "V": "Kedua tangan: telunjuk & jari tengah membentuk V saling berhadapan",
    "W": "Kedua tangan: telunjuk, tengah & manis membentuk W, sejajar",
    "X": "Tangan kanan: telunjuk ditekuk seperti kait · Tangan kiri: telapak menghadap depan",
    "Y": "Tangan kanan: ibu jari & kelingking membentang · Tangan kiri: mengepal",
    "Z": "Tangan kanan: telunjuk gambar huruf Z · Tangan kiri: telapak terbuka",
}
    ...
    with col_cam:
        ...
        ctx = webrtc_streamer(
            ...
            rtc_configuration=RTCConfiguration({"iceServers": [
                ...
                {"urls": ["turn:a.relay.metered.ca:80"], "username": "bisindo", "credential": "bisindo123"},
                {"urls": ["turn:a.relay.metered.ca:443"], "username": "bisindo", "credential": "bisindo123"},
            ]}),
...
# ==========================
    ...
    # Fungsi cek apakah overlay masih berjalan
    def is_overlay_running():
        if st.session_state.overlay_pid is None:
            return False
        try:
            proc = subprocess.Popen(
                ["tasklist", "/FI", f"PID eq {st.session_state.overlay_pid}"],
                stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                creationflags=subprocess.CREATE_NO_WINDOW if sys.platform=="win32" else 0
            )
            out, _ = proc.communicate()
            return str(st.session_state.overlay_pid) in out.decode()
        except:
            return False

    overlay_status = is_overlay_running()
    st.session_state.overlay_running = overlay_status

    # UI tombol
    col_btn, col_stop = st.columns([2, 1])

    with col_btn:
        if not st.session_state.overlay_running:
            if st.button(
                "🎬  AKTIFKAN OVERLAY SEKARANG",
                type="primary",
                use_container_width=True,
                help="Overlay subtitle akan muncul di layar kamu"
            ):
                try:
                    # Cari path overlay.py relatif ke main.py
                    overlay_path = _os.path.join(
                        _os.path.dirname(_os.path.abspath(__file__)),
                        "overlay.py"
                    )
                    if not _os.path.exists(overlay_path):
                        st.error("❌ overlay.py tidak ditemukan di folder yang sama dengan main.py")
                    else:
                        proc = subprocess.Popen(
                        ...
                        st.session_state.overlay_pid = proc.pid
                        st.session_state.overlay_running = True
                        st.success(f"✅ Overlay aktif! (PID: {proc.pid})")
                        st.rerun()
                except Exception as e:
                    st.error(f"❌ Gagal menjalankan overlay: {e}")
        else:
            st.markdown("""
            <div style="background:rgba(0,214,143,0.1);border:1px solid rgba(0,214,143,0.3);
                border-radius:12px;padding:14px 20px;text-align:center">
                <div style="font-size:15px;font-weight:700;color:#00D68F">
                    ✅ Overlay Sedang Berjalan
                </div>
                <div style="font-size:12px;color:#8FA3BF;margin-top:4px">
                    Lihat subtitle di layar kamu — tunjukkan gesture ke kamera
                </div>
            </div>
            """, unsafe_allow_html=True)

    with col_stop:
        if st.session_state.overlay_running:
            if st.button(
                "⏹ Stop Overlay",
                use_container_width=True,
                help="Matikan overlay"
            ):
                try:
                    if sys.platform == "win32":
                        subprocess.run(
                            ["taskkill", "/F", "/PID", str(st.session_state.overlay_pid)],
                            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
                        )
                    else:
                        import signal
                        _os.kill(st.session_state.overlay_pid, signal.SIGTERM)
                    st.session_state.overlay_pid = None
                    st.session_state.overlay_running = False
                    st.info("⏹ Overlay dihentikan.")
                    st.rerun()
                except Exception as e:
...
# ==========================
    ...
    st.markdown("""<div style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);
        ...
        <div style="margin-top:24px;padding-top:18px;border-top:1px solid rgba(255,255,255,0.06);
            ...
            <span style="font-size:11px;color:rgba(0,214,143,0.5)">— Second Byte · Artificial Intelligence · Universitas Mercu Buana · 2026</span>
        </div>
    </div>""", unsafe_allow_html=True)